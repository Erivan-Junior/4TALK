...
public class Teste1 {
	...
	public Teste1(){
		...
		try {
			...
			Fachada.criarIndividuo("maria", "132");
			Fachada.criarIndividuo("jose", "213");
		...
		try {
			...
			ArrayList<Mensagem> lista = Fachada.listarMensagensEnviadas("jose");
			for(Mensagem m : lista)
				System.out.println(m);

			System.out.println("\napagar a ultima mensagem enviada de jose");
			Mensagem ultima = lista.get(lista.size()-1);
			Fachada.apagarMensagem("jose", ultima.getId());
		...
		try {
			...
			for(Mensagem m : Fachada.espionarMensagens("admin", ""))
				System.out.println(m);
